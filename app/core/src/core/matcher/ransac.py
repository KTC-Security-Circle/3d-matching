...
def global_registration(
    ...
    return pipelines.registration.registration_ransac_based_on_feature_matching(
        ...
        estimation_method=pipelines.registration.TransformationEstimationPointToPoint(with_scaling=False),
        ...
        criteria=pipelines.registration.RANSACConvergenceCriteria(iteration, 0.999),
    )
...
def compute_feature_correspondences(
    ...
    *,
    ...
    Args:
        ...
        noise_ratio: ノイズ比率. 元の対応点数に対する偽対応点の割合 (例: 2.0 → 元の2倍の偽対応点を追加)
    ...
    if noise_ratio > 0:
        ...
        if n_noise > 0:
            ...
            src_indices = rng.integers(0, len(src.pcd_down.points), n_noise)
            tgt_indices = rng.integers(0, len(tgt.pcd_down.points), n_noise)
            ...
            rng.shuffle(corres_np)

    return o3d.utility.Vector2iVector(corres_np)


N_CORRES_THRESHOLD: Final = 3  # 対応点が3点未満の場合、変換推定をスキップして単位行列を返す
...
def compute_step_transformation(
    ...
    Returns:
        RegistrationResult: 推定された4x4変換行列を含む結果. 計算失敗時は単位行列(fitness=0.0)を返す.
    """
    ...
    if n_corres < N_CORRES_THRESHOLD:
        return res

    # 重複なしで3つの対応点をランダム選択
    idxs = rng.choice(n_corres, 3, replace=False)
    ...
    try:
        ...
        # 共分散行列: H = P^T @ Q (回転の推定に使用)  # noqa: ERA001
        h = np.dot(p.T, q)
        ...
        # 平行移動ベクトル: t = centroid_tgt - R @ centroid_src  # noqa: ERA001
        ...
        if not (np.isnan(trans).any() or np.isinf(trans).any()):
            res.transformation = trans

    except Exception as e:  # noqa: BLE001
        logger.warning("Kabschアルゴリズムの計算中に例外が発生: %s", e)
    else:
        return res
...
def evaluate_inlier_ratio(
    ...
    ソース点群に変換行列を適用し、対応するターゲット点との距離が 閾値(voxel_size * 1.5)以内となるペアの割合を計算する.
    ...
    # インライア率 = 距離が閾値未満のペア数 / 全対応点数  # noqa: ERA001
